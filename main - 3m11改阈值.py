    ...
    while True:
        try:
            if uwb.pure_data is not None:
                ...
                amplitude, index, energy_ratio = tools.sp(INPUT, 15, 20)
                ...
                for i in range(len(index)):
                    if(index[i]<=1.4):
                        if(amplitude[i]>2.5e-06 and energy_ratio[i] > 0):
                            Person += 1
                            Real.append(index[i])

                    elif(index[i]>1.4 and index[i]<=2.4):
                        if (amplitude[i] > 5.3e-07 and energy_ratio[i] > 0):
                            Person += 1
                            Real.append(index[i])

                    elif (index[i] > 2.4 and index[i] <= 3.4):
                        if (amplitude[i] > 4.08e-07 and energy_ratio[i] > 0):
                            Person += 1
                            Real.append(index[i])

                    elif (index[i] > 3.4 ):
                        if (amplitude[i] > 1.4e-07 and energy_ratio[i] > 0):
                            Person += 1
                            Real.append(index[i])
                    else:
